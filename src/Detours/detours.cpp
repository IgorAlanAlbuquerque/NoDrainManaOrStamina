...
int Detour_AssertExprWithFunctionName(int reportType, const char* filename, int linenumber, const char* FunctionName,
                                      const char* msg) {
    int nRet = 0;
    ...
    StringCchCatNA(szModuleNameWithFunctionName, ARRAYSIZE(szModuleNameWithFunctionName), ",",
                   ARRAYSIZE(szModuleNameWithFunctionName) - strlen(szModuleNameWithFunctionName) - 1);
    StringCchCatNA(szModuleNameWithFunctionName, ARRAYSIZE(szModuleNameWithFunctionName), FunctionName,
                   ARRAYSIZE(szModuleNameWithFunctionName) - strlen(szModuleNameWithFunctionName) - 1);
...
static bool detour_is_imported(PBYTE pbCode, PBYTE pbAddress) {
    ...
    __try {
        ...
        if (pbAddress >= ((PBYTE)pDosHeader +
            ...
            pbAddress <
                ((PBYTE)pDosHeader + pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress +
        ...
        // Finally, skip over a long jump if it is the target of the patch jump.
            ...
            if (pbCode[0] == 0xff && pbCode[1] == 0x25 &&
                *(UNALIGNED INT32*)&pbCode[2] == (UNALIGNED INT32)(pbCode + 0x1000)) {  // jmp [eip+PAGE_SIZE-6]
                ...
                DETOUR_TRACE(
                    ("%p->%p: OS patch encountered, reset back to long jump 5 bytes prior to target function.\n",
...
enum { SIZE_OF_JMP = sizeof(DETOUR_IA64_BUNDLE) };
...
inline PBYTE detour_skip_jmp(PBYTE pPointer, PVOID* ppGlobals) {
    ...
    //                     002024000200100b
    if ((pb[0].wide[0] & 0xfffffc000603ffff) == 0x002024000200100b && pb[0].wide[1] == 0x0004000000203008 &&
        pb[1].wide[0] == 0x001014180420180a && pb[1].wide[1] == 0x07000830c0203008 &&
        pb[2].wide[0] == 0x0000000100000010 && pb[2].wide[1] == 0x0080006000000200) {
        ULONG64 offset = ((pb[0].wide[0] & 0x0000000001fc0000) >> 18) |  // imm7b
...
inline PBYTE align4(PBYTE pValue) { return (PBYTE)(((ULONG)pValue) & ~(ULONG)3u); }
    ...
    if ((Opcode & 0xfbf08f00) == 0xf2400c00) {  // movw r12,#xxxx
        ...
        if ((Opcode2 & 0xfbf08f00) == 0xf2c00c00) {  // movt r12,#xxxx
            ...
            if (Opcode3 == 0xf8dcf000) {  // ldr  pc,[r12]
                PBYTE pbTarget = (PBYTE)(((Opcode2 << 12) & 0xf7000000) | ((Opcode2 << 1) & 0x08000000) |
                                         ((Opcode2 << 16) & 0x00ff0000) | ((Opcode >> 4) & 0x0000f700) |
                                         ((Opcode >> 15) & 0x00000800) | ((Opcode >> 0) & 0x000000ff));
                if (detour_is_imported(pbCode, pbTarget)) {
...
inline ULONG fetch_opcode(PBYTE pbCode) { return *(ULONG*)pbCode; }
...
PBYTE detour_gen_jmp_indirect(BYTE* pbCode, ULONG64* pbJmpVal) {
    ...
    jmpIndAddr.value = (((LONG64)pbJmpVal) & 0xFFFFFFFFFFFFF000) - (((LONG64)pbCode) & 0xFFFFFFFFFFFFF000);
        ...
        if ((Opcode2 & 0xffe003ff) == 0xf9400210) {  // ldr   x16, [x16, IAT]
            ...
            if (Opcode3 == 0xd61f0200) {  // br    x16
                ...
                /* https://static.docs.arm.com/ddi0487/bb/DDI0487B_b_armv8_arm.pdf
                    The ADRP instruction shifts a signed, 21-bit immediate left by 12 bits, adds it to the value of the
                program counter with the bottom 12 bits cleared to zero, and then writes the result to a general-purpose
                register. This permits the calculation of the address at a 4KB aligned memory region. In conjunction
                with an ADD (immediate) instruction, or a Load/Store instruction with a 12-bit immediate offset, this
                allows for the calculation of, or access to, any address within +/- 4GB of the current PC.

                PC-rel. addressing
                    This section describes the encoding of the PC-rel. addressing instruction class. The encodings in
                this section are decoded from Data Processing -- Immediate on page C4-226. Add/subtract (immediate) This
                section describes the encoding of the Add/subtract (immediate) instruction class. The encodings in this
                section are decoded from Data Processing -- Immediate on page C4-226. Decode fields Instruction page op
                    0 ADR
                    1 ADRP

                C6.2.10 ADRP
                    Form PC-relative address to 4KB page adds an immediate value that is shifted left by 12 bits, to the
                PC value to form a PC-relative address, with the bottom 12 bits masked out, and writes the result to the
                destination register. ADRP <Xd>, <label> imm = SignExtend(immhi:immlo:Zeros(12), 64);
                ...
                */
                ...
                address that is used for the load is calculated from a base register and an immediate offset. The
                Unsigned offset variant scales the immediate offset value by the size of the value accessed before
                adding it to the base register value.
...
static PVOID detour_alloc_region_from_lo(PBYTE pbLo, PBYTE pbHi) {
    ...
    for (; pbTry < pbHi;) {
        ...
        DETOUR_TRACE(("  Try %p => %p..%p %6lx\n", pbTry, mbi.BaseAddress, (PBYTE)mbi.BaseAddress + mbi.RegionSize - 1,
                      mbi.State));
...
PVOID WINAPI DetourAllocateRegionWithinJumpBounds(_In_ LPCVOID pbTarget, _Out_ PDWORD pcbAllocatedSize) {
    ...
    PVOID pbNewlyAllocated = detour_alloc_trampoline_allocate_new((PBYTE)pbTarget, pLo, pHi);
...
LONG WINAPI DetourTransactionBegin() {
    ...
    _Benign_race_begin_ if (s_nPendingThreadId != 0) { return ERROR_INVALID_OPERATION; }
    _Benign_race_end_
...
LONG WINAPI DetourTransactionCommit() { return DetourTransactionCommitEx(NULL); }
        ...
        if (o->fIsRemove) {
            ...
            *o->ppbPointer = o->pbTarget;
            ...
            DETOUR_TRACE(("detours: pbTramp =%p, pbRemain=%p, pbDetour=%p, cbRestore=%u\n", o->pTrampoline,
                          o->pTrampoline->pbRemain, o->pTrampoline->pbDetour, o->pTrampoline->cbRestore));
            ...
            DETOUR_TRACE(
                ("detours: pbTarget=%p: "
                 ...
                 o->pbTarget, o->pbTarget[0], o->pbTarget[1], o->pbTarget[2], o->pbTarget[3], o->pbTarget[4],
                 o->pbTarget[5], o->pbTarget[6], o->pbTarget[7], o->pbTarget[8], o->pbTarget[9], o->pbTarget[10],
                 o->pbTarget[11]));

#ifdef DETOURS_IA64
            ((DETOUR_IA64_BUNDLE*)o->pbTarget)->SetBrl((UINT64)&o->pTrampoline->bAllocFrame);
            ...
            *o->ppbPointer = o->pTrampoline->rbCode;
            ...
            DETOUR_TRACE(
                ("detours: pbTramp =%p: "
                 ...
                 "%02x %02x %02x %02x "
                 "%02x %02x %02x %02x\n",
                 o->pTrampoline, o->pTrampoline->rbCode[0], o->pTrampoline->rbCode[1], o->pTrampoline->rbCode[2],
                 o->pTrampoline->rbCode[3], o->pTrampoline->rbCode[4], o->pTrampoline->rbCode[5],
                 o->pTrampoline->rbCode[6], o->pTrampoline->rbCode[7], o->pTrampoline->rbCode[8],
                 o->pTrampoline->rbCode[9], o->pTrampoline->rbCode[10], o->pTrampoline->rbCode[11]));
                        ...
                        cxt.DETOURS_EIP = (DETOURS_EIP_TYPE)((ULONG_PTR)o->pbTarget +
                                                             detour_align_from_trampoline(
                                                                 ...
                                                                 (BYTE)(cxt.DETOURS_EIP -
                                                                        (DETOURS_EIP_TYPE)(ULONG_PTR)o->pTrampoline)));

                        SetThreadContext(t->hThread, &cxt);
                    }
                } else {
                    if (cxt.DETOURS_EIP >= (DETOURS_EIP_TYPE)(ULONG_PTR)o->pbTarget &&
                        cxt.DETOURS_EIP < (DETOURS_EIP_TYPE)((ULONG_PTR)o->pbTarget + o->pTrampoline->cbRestore)) {
                        cxt.DETOURS_EIP =
                            (DETOURS_EIP_TYPE)((ULONG_PTR)o->pTrampoline +
                                               detour_align_from_target(
                                                   o->pTrampoline,
                                                   (BYTE)(cxt.DETOURS_EIP - (DETOURS_EIP_TYPE)(ULONG_PTR)o->pbTarget)));
...
LONG WINAPI DetourAttachEx(_Inout_ PVOID* ppPointer, _In_ PVOID pDetour, _Out_opt_ PDETOUR_TRAMPOLINE* ppRealTrampoline,
                           _Out_opt_ PVOID* ppRealTarget, _Out_opt_ PVOID* ppRealDetour) {
    LONG error = NO_ERROR;
    ...
    if (RtlIsEcCode(reinterpret_cast<DWORD64>(*ppPointer))) {
        DETOUR_TRACE(
            ("*ppPointer is an Arm64EC address (ppPointer=%p). "
    ...
    DETOUR_TRACE(
        ("detours: pbTarget=%p: "
         ...
         "%02x %02x %02x %02x "
         "%02x %02x %02x %02x\n",
         pbTarget, pbTarget[0], pbTarget[1], pbTarget[2], pbTarget[3], pbTarget[4], pbTarget[5], pbTarget[6],
         pbTarget[7], pbTarget[8], pbTarget[9], pbTarget[10], pbTarget[11]));
    DETOUR_TRACE(
        ("detours: pbTramp =%p: "
         "%02x %02x %02x %02x "
         "%02x %02x %02x %02x "
         "%02x %02x %02x %02x\n",
         pTrampoline, pTrampoline->rbCode[0], pTrampoline->rbCode[1], pTrampoline->rbCode[2], pTrampoline->rbCode[3],
         pTrampoline->rbCode[4], pTrampoline->rbCode[5], pTrampoline->rbCode[6], pTrampoline->rbCode[7],
         pTrampoline->rbCode[8], pTrampoline->rbCode[9], pTrampoline->rbCode[10], pTrampoline->rbCode[11]));
...
LONG WINAPI DetourDetach(_Inout_ PVOID* ppPointer, _In_ PVOID pDetour) {
    ...
    pDetour = (PBYTE)DetourCodeFromPointer(ppldDetour, &pDetourGlobals);
    PDETOUR_TRAMPOLINE pTrampoline = (PDETOUR_TRAMPOLINE)DetourCodeFromPointer(ppldTrampo, &pTrampoGlobals);
    DETOUR_TRACE(("  ppldDetour=%p, code=%p [gp=%p]\n", ppldDetour, pDetour, pDetourGlobals));
    ...
    DETOUR_TRACE(
        ("detours:  &bMovlTargetGp  =%p [%p]\n", &pTrampoline->bMovlTargetGp, pTrampoline->bMovlTargetGp.GetMovlGp()));
    DETOUR_TRACE(("detours:  &rbCode         =%p [%p]\n", &pTrampoline->rbCode,
                  ((DETOUR_IA64_BUNDLE&)pTrampoline->rbCode).GetBrlTarget()));
    DETOUR_TRACE(("detours:  &bBrlRemainEip  =%p [%p]\n", &pTrampoline->bBrlRemainEip,
                  pTrampoline->bBrlRemainEip.GetBrlTarget()));
    DETOUR_TRACE(
        ("detours:  &bMovlDetourGp  =%p [%p]\n", &pTrampoline->bMovlDetourGp, pTrampoline->bMovlDetourGp.GetMovlGp()));
    DETOUR_TRACE(
        ("detours:  &bBrlDetourEip  =%p [%p]\n", &pTrampoline->bCallDetour, pTrampoline->bCallDetour.GetBrlTarget()));
...
#define DETOUR_PAGE_EXECUTE_ALL (PAGE_EXECUTE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE | PAGE_EXECUTE_WRITECOPY)

#define DETOUR_PAGE_NO_EXECUTE_ALL (PAGE_NOACCESS | PAGE_READONLY | PAGE_READWRITE | PAGE_WRITECOPY)
...
static DWORD DetourPageProtectAdjustExecute(_In_ DWORD dwOldProtect, _In_ DWORD dwNewProtect)
...
_Success_(return != FALSE) BOOL WINAPI
    DetourVirtualProtectSameExecuteEx(_In_ HANDLE hProcess, _In_ PVOID pAddress, _In_ SIZE_T nSize,
                                      _In_ DWORD dwNewProtect, _Out_ PDWORD pdwOldProtect)
...
// When "restoring" page protection, there is no need to use this function.
{
    ...
    return VirtualProtectEx(hProcess, pAddress, nSize, DetourPageProtectAdjustExecute(mbi.Protect, dwNewProtect),
                            pdwOldProtect);
}

_Success_(return != FALSE) BOOL WINAPI
    DetourVirtualProtectSameExecute(_In_ PVOID pAddress, _In_ SIZE_T nSize, _In_ DWORD dwNewProtect,
