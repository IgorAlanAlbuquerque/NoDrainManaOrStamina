...
PDETOUR_SYM_INFO DetourLoadImageHlp(VOID) {
    ...
    if (symInfo.pfImagehlpApiVersionEx == NULL || symInfo.pfSymInitialize == NULL ||
        symInfo.pfSymLoadModule64 == NULL || symInfo.pfSymGetModuleInfo64 == NULL || symInfo.pfSymFromName == NULL) {
        goto abort;
    }
...
#pragma prefast(suppress : 28752, "We don't do the unicode conversion for LoadLibraryExA.")
    ...
    if (pSymInfo->pfSymLoadModule64(pSymInfo->hProcess, NULL, (PCHAR)pszModule, NULL, (DWORD64)hModule, 0) == 0) {
...
HMODULE WINAPI DetourEnumerateModules(_In_opt_ HMODULE hModuleLast) {
    ...
    for (;; pbLast = (PBYTE)mbi.BaseAddress + mbi.RegionSize) {
        ...
        __try {
            ...
            if (pDosHeader->e_magic != IMAGE_DOS_SIGNATURE || (DWORD)pDosHeader->e_lfanew > mbi.RegionSize ||
...
#pragma warning(suppress : 6011)  // GetModuleHandleW(NULL) never returns NULL.
        ...
        PIMAGE_EXPORT_DIRECTORY pExportDir = (PIMAGE_EXPORT_DIRECTORY)RvaAdjust(
            pDosHeader, pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);
        ...
        for (DWORD nFunc = 0; nFunc < pExportDir->NumberOfFunctions; nFunc++) {
            ...
            for (DWORD n = 0; n < pExportDir->NumberOfNames; n++) {
                if (pwOrdinals[n] == nFunc) {
                    pszName = (pdwNames != NULL) ? (PCHAR)RvaAdjust(pDosHeader, pdwNames[n]) : NULL;
                    break;
                }
            }
...
#pragma warning(suppress : 6011)  // GetModuleHandleW(NULL) never returns NULL.
        ...
        PIMAGE_IMPORT_DESCRIPTOR iidp = (PIMAGE_IMPORT_DESCRIPTOR)RvaAdjust(
            pDosHeader, pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress);
...
static BOOL CALLBACK DetourEnumerateImportsThunk(_In_ PVOID VoidContext, _In_ DWORD nOrdinal, _In_opt_ PCSTR pszFunc,
                                                 _In_opt_ PVOID* ppvFunc) {
    _DETOUR_ENUMERATE_IMPORTS_THUNK_CONTEXT const* const pContext =
        (_DETOUR_ENUMERATE_IMPORTS_THUNK_CONTEXT*)VoidContext;
...
BOOL WINAPI DetourEnumerateImports(_In_opt_ HMODULE hModule, _In_opt_ PVOID pContext,
    ...
    return DetourEnumerateImportsEx(hModule, (PVOID)&context, pfImportFile, &DetourEnumerateImportsThunk);
}
...
#pragma warning(suppress : 6011)  // GetModuleHandleW(NULL) never returns NULL.
        ...
        PIMAGE_SECTION_HEADER pSectionHeaders =
            (PIMAGE_SECTION_HEADER)((PBYTE)pNtHeader + sizeof(pNtHeader->Signature) + sizeof(pNtHeader->FileHeader) +
